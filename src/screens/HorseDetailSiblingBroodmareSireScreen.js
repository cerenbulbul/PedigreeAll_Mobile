...
export function HorseDetailSiblingBroodmareSireScreen({ route, navigation }) {
              ...
              <DataTable>
                ...
                {getSiblingBroodmareSire.HORSE_INFO_LIST.map((item, index) => (
                  ...
                  <DataTable.Row centered={true} key={index}>
                    ...
                    <DataTable.Cell onPress={() => { alertDialog("Sire", item.MOTHER_NAME) }} style={{ width: 100, height: 'auto', marginLeft: 20 }}>{item.MOTHER_NAME}</DataTable.Cell>
                    <DataTable.Cell onPress={() => { alertDialog("Dam", item.FATHER_NAME) }} style={{ width: 100, height: 'auto', marginLeft: 20 }}>{item.FATHER_NAME}</DataTable.Cell>
